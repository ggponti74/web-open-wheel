...
const STANDINGS_URL = `https://www.fiaformula2.com/en/standings/${YEAR}/drivers`;

async function scrapeF2Drivers() {
  try {
    ...
    if (!table) {
      ...
      writeFileSync('public/data/f2-drivers.json', 'null');
      return;
    }
    ...
    const standings = rows
      .map((row) => {
        ...
        // First cell is like "1N. Tsolov" — leading digits are the position,
    ...
    if (standings.length === 0) {
      console.error('No driver rows parsed from F2 standings table');
      writeFileSync('public/data/f2-drivers.json', 'null');
      return;
    }

    writeFileSync('public/data/f2-drivers.json', JSON.stringify(standings, null, 2));
    console.log(`Wrote ${standings.length} drivers to f2-drivers.json`);
  } catch (e) {
    console.error('F2 drivers scrape failed:', e.message);
  }
}

await scrapeF2Drivers();
