...
const STANDINGS_URL = `https://www.fiaformula3.com/en/standings/${YEAR}/teams`;
...
async function scrapeF3Teams() {
  try {
    ...
    if (!table) {
      ...
      writeFileSync("public/data/f3-teams.json", "null");
      return;
    }
    ...
    if (standings.length === 0) {
      console.error("No team rows parsed from F3 standings table");
      writeFileSync("public/data/f3-teams.json", "null");
      return;
    }

    writeFileSync(
      "public/data/f3-teams.json",
      JSON.stringify(standings, null, 2),
    );
    console.log(`Wrote ${standings.length} teams to f3-teams.json`);
  } catch (e) {
    console.error("F3 teams scrape failed:", e.message);
  }
}
